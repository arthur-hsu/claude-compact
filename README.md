...
1. **PreCompact hook** (best-effort) — before auto-compact fires, extracts structured state from the transcript via Sonnet and writes `<cwd>/.claude/HANDOFF-<sid>.md`. Falls back to mechanical extract if Sonnet fails.
...
- `hooks/pre-compact.mjs` — PreCompact: transcript digest → Sonnet → HANDOFF-<sid>.md
